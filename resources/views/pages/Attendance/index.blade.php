                        ...
                        <tbody>
                            @foreach ($Attendances as $Attendance)
                                <tr>
                                    ...
                                    <td style=" background-color: #D0DEF6; font-weight:bolder;">
                                        {{ $Attendance->attendance_status }}</td>
                                    <td>{{ $Attendance->create_by }}</td>
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('Attendance.edit', $Attendance->id) }}"
                                                style="margin: 3px;" title="تعديل" class="btn btn-info btn-sm"
                                                role="button" aria-pressed="true"><i class="fa fa-edit"></i></a>
                                            <button type="button" class="btn btn-danger btn-sm" style="margin: 3px;"
                                                data-toggle="modal"
                                                data-target="#Delete_Attendance{{ $Attendance->id }}" title="حـذف"><i
                                ...
                                <!-- delete_modal_Grade -->
                                <div class="modal fade" id="Delete_Attendance{{ $Attendance->id }}" tabindex="-1"
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 style="font-family: 'Cairo', sans-serif;" class="modal-title"
                                                    ...
                                                    حـذف تحضـير الطـالـب <label
                                                        style="color: #5686E0; font-weight:bolder; font-size: 20px;">{{ $Attendance->students->name }}</label>
                                                </h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                    aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                            </div>
                                            <div class="modal-body">
                                                <form action="{{ route('Attendance.destroy', 'test') }}"
                                                    method="post">
                                                    {{ method_field('Delete') }}
                                                    @csrf
                                                    <h5 style="font-family: 'Cairo', sans-serif;"> هل انت متاكد من عملية
                                                        حـذف تحضيـر الطـالـب ؟ </h5>
                                                    <input id="Name" type="text" name="Name"
                                                        class="form-control"
                                                        style="text-align: center; font-weight:bolder; font-size: 20px;"
                                                        value="{{ $Attendance->students->name }}" disabled>
                                                    <input id="id" type="hidden" name="id"
                                                        class="form-control" value="{{ $Attendance->id }}">
