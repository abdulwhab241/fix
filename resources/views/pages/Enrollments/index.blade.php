...
<div class="box-body">
    ...
    <button type="button" class="btn btn-info btn-flat" title="تسجيل الطالب الذي تم إلغاء تسجيله"
        style="margin: 5px; padding: 5px;" data-toggle="modal" data-target="#exampleModal">
            ...
            <td>
                <div class="btn-group">
                    <button type="button" style="margin: 3px;" class="btn btn-danger btn-sm"
                        data-toggle="modal" data-target="#delete{{ $Enrollment->id }}"
                        title="إلـغاء تسجيـل الطالب">إلـغاء تسجيـل الطالب </button>
                </div>
            </td>
        </tr>

        <!-- Deleted inFormation Student -->
        <div class="modal fade" id="delete{{ $Enrollment->id }}" tabindex="-1"
            aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-warning">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 style="font-family: 'Cairo', sans-serif;" class="modal-title"
                            id="exampleModalLabel">إلـغاء تسجيـل الطـالـب</h5>
                        <button type="button" class="close" data-dismiss="modal"
                            aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('Enrollments.destroy', 'test') }}"
                            method="post">
                            @csrf
                            @method('DELETE')
                            <input type="hidden" name="id"
                                value="{{ $Enrollment->id }}}">
                            <h5 style="font-family: 'Cairo', sans-serif; font-size:20px;">هل انت
                                متاكد من عملية الغاء تسجيـل الطـالـب ؟</h5>
                            <input type="text" readonly
                                value="{{ $Enrollment->student->name }}"
                                style=" font-weight: bolder; font-size:20px; text-align: center;"
                                class="form-control">

                            <div class="modal-footer">
                                <button type="button" class="btn btn-outline"
                                    data-dismiss="modal">إغلاق</button>
                                <button type="submit" class="btn btn-danger">تـأكيـد </button>
                            </div>
                        </form>
                    </div>
                </div>
